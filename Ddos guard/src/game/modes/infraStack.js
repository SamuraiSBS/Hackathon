export const infraStackMode = {
    ...
    if (input.tap) {
      ...
      if (overlapWidth < 28) {
        finish({
          result: 'defeat',
          score: state.score,
          reason: 'Слой лег мимо и стек потерял устойчивость.',
        });
        return;
      }
      ...
      if (state.layers >= 10) {
        finish({
          ...
          reason: 'Башня из сетевых слоев выдержала нагрузку.',
        });
        return;
      }
  ...
  onTimeout({ state, finish }) {
    finish({
      ...
      reason:
        state.layers >= 7
          ? 'Башня успела набрать устойчивость до завершения времени.'
          : 'Для устойчивой архитектуры не хватило нескольких слоев.',
    });
  },
};
